...
FILE_PATTERN="\.(js|es6|jsx|tsx|ts|scss)$"
...
STYLELINT_BIN=$ROOT_DIR/node_modules/.bin/stylelint
...
STYLELINT_RETVAL=0
if [ "$STYLE_STAGED_FILES" != "" ]; then
    $STYLELINT_BIN $STYLE_STAGED_FILES > ./tmp/stylelint.txt
    STYLELINT_RETVAL=$?
fi
...
if [ $STYLELINT_RETVAL -ne 0 ]; then
    cat ./tmp/stylelint.txt
fi

RETVAL=$(($ESLINT_RETVAL+$STYLELINT_RETVAL))
rm -rf ./tmp
rm -r $TMP_STAGING
exit $RETVAL